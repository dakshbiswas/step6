...
public class DataMungerTest {
	...
	public void testWithWhereThreeConditionsGroupBy() throws FileNotFoundException {
		...
		HashMap dataSet = query.executeQuery(
				"select winner,count(winner) from data/ipl.csv where season >= 2013 or toss_decision != bat and city = Bangalore group by winner");

		boolean totalColumnsExpected = dataSet.entrySet().iterator().next().toString().split("=").length == 2;
		boolean dataExpectedStatus = dataSet.entrySet().size() == 13;

		assertNotNull("testWithWhereThreeConditionsGroupBy() : Empty Dataset returned", dataSet);

		assertEquals("testWithWhereThreeConditionsGroupBy() : Total number of columns should be 2", true,
				totalColumnsExpected);
		assertEquals(
